...
@Component({
  selector: 'app-skulevel-orders',
  templateUrl: './skulevel-orders.component.html',
  styleUrls: ['./skulevel-orders.component.scss']
})
export class SkulevelOrdersComponent implements OnInit {
  skulevelcustomerOrder?: ORDER.Orderitemgrp[] = [];
  packagepaginate?: ORDER.OrdersPaginate = {};
  currentIndex = -1;
  addAction = false;
  viewAction = false;
  orderStatus = '';
  title = 'SKU Level Orders';
  /// Paginate ////
  search = '';
  page = 1;
  count = 0;
  ...
  list(): void {
    ...
    this.invoiceservice.getSkulevelOrders({ ...params, ...additionalParams }).subscribe({
      next: (customerOrders) => {
  ...
  download() {
    ...
    this.invoiceservice.downloadSkulevelOrders({ ...params, ...additionalParams }).subscribe({
      next: (resp) => {
