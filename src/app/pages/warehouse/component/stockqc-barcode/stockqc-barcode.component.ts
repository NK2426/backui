...
@Component({
  selector: 'app-stockqc-barcode',
  templateUrl: './stockqc-barcode.component.html',
  styleUrls: ['./stockqc-barcode.component.scss'],
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class StockqcBarcodeComponent implements OnInit {
  ...
  verifyQRcode() {
    ...
    if (insbulkitemscheck > 0) {
      ...
      this.isupdate = false;
      this.barcodeservice.view(this.scanbarcode).subscribe({
        next: (items: any) => {
          this.barcode = items;
          ...
          if (this.isupdate == false) {
            ...
            this.disputeForm.get('v_name').setValue(items.v_name);
  ...
  onSelectedFile_image(event: any) {
    if (event.target.files.length > 0) {
      ...
      if (!mimeType.match('image.*')) {
        ...
        // console.log('solemon ->', this.addfile)
        this.compressImage
          .compress(file)
          .pipe(take(1))
