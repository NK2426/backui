...
import { NgbDateStruct, NgbModal, NgbModule, NgbPaginationModule } from '@ng-bootstrap/ng-bootstrap';
...
@Component({
  selector: 'app-outward-invoice',
  templateUrl: './outward-invoice.component.html',
  styleUrls: ['./outward-invoice.component.scss'],
  standalone: true,
...
export class OutwardInvoiceComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private cdr: ChangeDetectorRef, private toast: ToastService, private utils: UtilsService,
    private invoiceService: InvoiceService, private porderservice: PurchaseorderService, private formBuilder: FormBuilder, private modelservice: NgbModal) { }
      ...
      if (confirm('Are you sure you want to proceed?')) {
        ...
        this.invoiceService.generateAWB(this.invoiceDetail.invoiceno, data).subscribe({
          next: resp => {
            ...
            this.toast.failure('AWB Not Generate... Try again later...');
            this.submit = false;
          }
        })
      }
  ...
  cancelShipment(): void {
    ...
    if (confirm('Are you sure you want to proceed?')) {
      ...
      this.invoiceService.cancelAWB(this.invoiceDetail.invoiceno, data).subscribe({
        next: resp => {
          if (resp.awbGenerateStatus !== 'Yes') {
            let reason = resp.reason || 'Try again later...'
    ...
    this.invoiceService.downloadinvoice(this.invoiceNumber).subscribe({
      next: resp => {
        if (resp.message == 'Success') {
          ...
          //link.download = path;
          document.body.appendChild(link);
          //console.log(environment.PDF_BASE_URL + 'INV_' + this.invoiceNumber + '.pdf')
          link.click();
  ...
  /*removeItem(uuid: string): void {
    ...
    modalRef.componentInstance.confirmationBoxTitle = 'Cancel Confirmation';
    modalRef.componentInstance.confirmationMessage = 'Do you want to remove this item.?';

    let data = { orderitemuuid: uuid }
    modalRef.result.then(() => {
      this.invoiceService.removeItem(this.invoiceDetail.invoiceno, data).subscribe({
        next: resp => {
          if (resp.status === 'success') {
            this.toast.success(`${'Item Cancelled Successfully'} `);
            this.list(this.invoiceDetail.invoiceno)
            this.submit = false;
          }
          else {
            let reason = resp.message || 'Try again later...'
            this.toast.failure('Item Not Cancelled...' + reason);
            this.submit = false;
          }
        }, error: err => {
          this.toast.failure('Shipment Not Cancelled... Try again later...');
