...
export class BlogsComponent {
  warehouses?: Blog[];
  departmentpaginate?: BlogPaginate = {};
  currentWarehouse: Blog = {};
  currentIndex = -1;
  addAction = false;
  viewAction = false;
  ...
  viewEdit(link: string, id: any) {
    if (link === "edit") {
      this.router.navigate(['catalog/edit-blog/' + id]);
    }
    if (link === "view") {
      this.router.navigate(['catalog/view-blog/' + id]);
    }
  }

  deleteWarehouse(Warehouse: Blog): void {
    // console.log('inside');
