...
// import { Product, Productimage } from 'src/app/pages/catalog/models/product';
import { Product, Productimage } from 'src/app/pages/category-head/models/product';
import { Productparameters } from 'src/app/pages/category-head/models/productparameters';
import { Productvariants } from 'src/app/pages/category-head/models/productvariants';
import { Vendor } from 'src/app/pages/category-head/models/purchaseorder';
...
@Component({
  selector: 'app-mappingparams',
  templateUrl: './mappingparams.component.html',
  styleUrls: ['./mappingparams.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MappingparamsComponent implements OnInit {
  selproduct: Product = {};
  uuid: string = '';
  ...
  submit() {
    ...
    if (allow == true) {
      ...
      modalRef.componentInstance.confirmationMessage =
        ' Do you want to do Publish this product?';
      modalRef.result.then(
        (parameterResponse) => {
          if (this.selproduct.vendormapping == null || this.selproduct.vendormapping.status == -1) {
            this.toast.failure('Please map the  variant & publish the product');
            return;
          }
  ...
  reqdraft() {
    ...
    this.productservice.mapping(formdata, this.uuid).subscribe({
      next: (resp) => {
        ...
        this.router.navigate(['/catalog/view/' + this.uuid]);
        //this.cdr.detectChanges();
        //this.router.navigate(['/category-head/products/mapping/'+this.uuid]);
      },
      error: (err) => {
