...
export class ViewSubcategoriesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (uuid) {
      this.pageuuid = uuid;
      this.subcategoriesService.find(uuid).subscribe({
        next: (resp) => {
  ...
  savecategory() {
   ...
   this.formData.get('status').value 
    ...
    if (this.selectedCategory.id) {
      formd.append('id', this.selectedCategory.id);
      msg = 'Updated';
    }
    this.subcategoriesService.updateimg(formd).subscribe({
      next: (resp) => {
        this.addfile = '';
        this.submit = false;
        this.show = false;
        ...
        this.selectedCategory = resp;
        this.toast.success('Subcategory Successfully ' + msg);
        // location.reload();
        // this.cdr.detectChanges();
      },
      error: (err) => {
