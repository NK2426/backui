...
export class LayoutService {
  ...
  public layoutConfigSubject: BehaviorSubject<ILayout> = new BehaviorSubject<ILayout>(
    this.getLayoutConfig(this.getBaseLayoutTypeFromRouteOrSessionStorage())
  );
  ...
  getBaseLayoutTypeFromRouteOrSessionStorage(): LayoutType {
    ...
    return this.getBaseLayoutTypeFromSessionStorage();
  }

  getBaseLayoutTypeFromSessionStorage(): LayoutType {
    if (sessionStorage) {
      const layoutType = sessionStorage.getItem(BASE_LAYOUT_TYPE_LOCAL_STORAGE_KEY);
      if (layoutType) {
  ...
  getLayoutConfig(layoutType: LayoutType): ILayout {
    const storedLayoutType = this.getBaseLayoutTypeFromSessionStorage();
    if (layoutType && storedLayoutType) {
      const configInString = sessionStorage.getItem(`${layoutType}-${LAYOUT_CONFIG_LOCAL_STORAGE_KEY}`);
  ...
  setBaseLayoutType(layoutType: LayoutType) {
    ...
    if (sessionStorage) {
      sessionStorage.setItem(BASE_LAYOUT_TYPE_LOCAL_STORAGE_KEY, layoutType);
      sessionStorage.setItem(`${layoutType}-${LAYOUT_CONFIG_LOCAL_STORAGE_KEY}`, JSON.stringify(config));
    }
    // document.location.reload();
  }

  saveBaseConfig(config: ILayout) {
    const baseLayoutType = this.getBaseLayoutTypeFromSessionStorage();
    if (sessionStorage) {
      sessionStorage.setItem(`${baseLayoutType}-${LAYOUT_CONFIG_LOCAL_STORAGE_KEY}`, JSON.stringify(config));
    }
  ...
  resetBaseConfig() {
    const layoutType = this.getBaseLayoutTypeFromSessionStorage;

    if (sessionStorage) {
      sessionStorage.removeItem(`${layoutType}-${LAYOUT_CONFIG_LOCAL_STORAGE_KEY}`);
    }
