...
import { ActivatedRoute, ActivatedRouteSnapshot, ActivationEnd, ActivationStart, ChildActivationEnd, ChildActivationStart, GuardsCheckEnd, GuardsCheckStart, NavigationCancel, NavigationEnd, NavigationError, NavigationStart, ResolveEnd, ResolveStart, RouteConfigLoadEnd, RouteConfigLoadStart, Router, RouterState, RouterStateSnapshot, RoutesRecognized, Scroll,Event } from '@angular/router';
...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  constructor(private layout: LayoutService, private route: ActivatedRoute,
    ...
    this.routestate = snapshotObj.url;    
    router.events.subscribe((event: Event) => {
     if (event instanceof NavigationEnd) {
        this.routestate=event.url
        
        this.minStatus=this.routestate.split('/')[2] === 'create-po' ? true : false
        this.setMinimize();
      }
    });
    ////console.log(this.routestate);
    this.minStatus=this.routestate.split('/')[2] === 'create-po' ? true : false
    ////console.log(this.minStatus,"=>minstatus")
  }
  ...
  updateProps(config: ILayout) {
    ...
    if (this.appSidebarDefaultMinimizeDesktopEnabled) {
      ...
      this.appSidebarDefaultMinimizeDesktopDefault = this.minStatus
